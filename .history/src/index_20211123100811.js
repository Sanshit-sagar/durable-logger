...
function UnprocessableEntityError(message) { 
    return new Response(UNPROCESSABLE_ENTITY_STR || message,  { status: 409 });
}
function NotFoundError(message) {
    return new Response(RESOURCE_NOT_FOUND_STR || message, { status: 404 })
}
function MethodNotAllowedError(message) {
    return new Response(METHOD_NOT_ALLOWED_STR || message, { status: 405 })
}
